...
class ImageBackground(FloatLayout):
    ...
    def yes_button_clicked(self, instance):
        ...
        if not self.yes_button.disabled:
            self.hide_buttons()
            self.show_image_on_screen(os.path.join(IMAGE_DIRECTORY, 'WIP-Instructions.png'))
            threading.Thread(target=self.predict_and_show, args=(os.path.join('testImage.jpg'),)).start()
